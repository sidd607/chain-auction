...
function closeAuction(closeBidding) {
  ...
  // by default we mark the listing as RESERVE_NOT_MET
    if (
    listing.auctionType == 'First_Price' ||
    listing.auctionType == 'First_Sealed'
  ) {
...
function makeOffer(offer) {
    return getParticipantRegistry('org.acme.product.auction.Member')
    .then(function(particiantRegistry) {
        return particiantRegistry.get(offer.member.email);
    })
    ...
    .catch(function(error) {
        throw new Error('Participant doesnot exist');
    });
}
